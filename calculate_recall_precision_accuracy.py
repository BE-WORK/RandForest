...
def calculate_recall_precision_accuracy():
    ...
    csv_read_file = file("result.csv", 'rb')
    csv_write_file = file("recall_precision_accuracy.csv", 'ab')
    ...
    for key in page_true:
        ...
        page_false_positive_ratio[key] = 1.0 * (page_all[key] - page_true_positive[key]) / page_true[key]  # 计算假阳率：被预测为正的负样本数/负样本实际数
        weight[key] = 1.0 * page_true[key] / total_sample
        writer.writerow(
            {'page name': key, 'page TPR': page_true_positive_ratio[key],
             'page FPR': page_false_positive_ratio[key], 'precision': page_precision, 'recall': page_recall, 'weight': weight[key]})
